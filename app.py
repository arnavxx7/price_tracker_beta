...
def search_query(q: str):
    ...
    log.info("Fastapi server received this query from nextjs: ", q)
    if not q:
        log.error(f"Received none as search query: {q}")
        return {
            "status": "error",
    ...
    if is_url(q):
        ...
        log.info(f"Received an url: {q}")
        url = q
    else:
        print("Received text")
        log.info(f"Received text: {q}")
        country_code = "in"
    ...
    if check_url:
        ...
        log.info("Product url detected")
        # match = re.search(r'/dp/([A-Z0-9]{10})', url)
        # asin =  match.group(1) if match else None
        product_info = amzn_product_info_scraper(html_content, url)

        if len(product_info)==0:
            log.error(f"{product_info.get("asin")}: Product information was not fetched due to captcha page or other reason, unable to update.")
    ...
    # Search url
        ...
        while current_url and current_page<=max_pages:
            ...
            log.info(f"Scraping search page {current_page}/{max_pages}: {current_url}")
            ...
            if not html_content or not html_content.content:
                ...
                log.error("Received empty html or failed to fetch search page")
                break
            ...
            if not products:
                ...
                log.error(f"No products found on page {current_page} (or page was blocked)")
                break

            print(f"[INFO] Found {len(products)} products on page {current_page}")
            log.info(f"Found {len(products)} products on page {current_page}")
        ...
        log.info(f"Search completed. Total products found: {len(all_products)}\n")
